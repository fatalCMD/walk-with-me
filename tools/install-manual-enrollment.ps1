...
$staging = (Resolve-Path -LiteralPath (Join-Path $projectRoot 'build/manual-enrollment-update')).Path
...
$backup = Join-Path $projectRoot ('backups/installed-pre-manual-enrollment-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
$pattern='(?im)^([ \t]*bAutoDiscover[ \t]*=[ \t]*)(?:true|false)([ \t]*(?:;[^\r\n]*)?)\r?$'
$matches=[regex]::Matches($iniText,$pattern)
if($matches.Count -ne 1){throw "Expected one bAutoDiscover setting. Previous INI is backed up at $backup"}
$updated=[regex]::Replace($iniText,$pattern,[System.Text.RegularExpressions.MatchEvaluator]{param($match) $match.Value -replace '(?i)(?<=\=)(\s*)(true|false)', '${1}false'})
[IO.File]::WriteAllText($ini,$updated,(New-Object System.Text.UTF8Encoding($false)))
if([IO.File]::ReadAllText($ini) -cne $updated){throw 'Enrollment setting verification failed'}
$receipt=@{destination=$target;backup=$backup;files=$written;settingsChanged=@('General.bAutoDiscover=false');otherSettingsPreserved=$true;time=(Get-Date -Format o)}
