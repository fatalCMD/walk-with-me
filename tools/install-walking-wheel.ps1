...
$staging = (Resolve-Path -LiteralPath (Join-Path $projectRoot 'build/walking-wheel-update')).Path
...
$backup = Join-Path $projectRoot ('backups/installed-pre-walking-wheel-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
